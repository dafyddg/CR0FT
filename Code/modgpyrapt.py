...
# wavfile_path = "/home/gibbon/Desktop/PyRapt/pyrapt-master/pyrapt/jiayan-5s.wav"
...
def _calculate_params(param, original_audio, downsampled_audio=None):
    ...
    param.hanning_window_vals = np.hanning(param.hanning_window_length)
...
#        denominator = math.sqrt(denominator_base * denominator_lag)
        denominator = (denominator_base * denominator_lag)**0.5
    else:
...
def _get_peak_lag_val(lag_results, lag_index, params):
    ...
    # parabolic_func = np.polyfit(x_vals, y_vals, 2)
...
def _get_rms_ratio(frame_idx, params):
    ...
    rms_curr = (float(curr_sum) / float(hanning_win_len))**0.5
    rms_prev = (float(prev_sum) / float(hanning_win_len))**0.5
